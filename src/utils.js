...
const util = {
    ...
    getProjectPath(document) {
        ...
        // 取workspaceFolders下目录的公共部分作为根目录
        ...
        while (workspaceFolders.length > 0) {
            let shiftStr = workspaceFolders.shift()
            if (!rootPathString) {
                rootPathString = shiftStr
            } else {
                let i = 0, combineStr = ""
                while (rootPathString.length > 0 && shiftStr.length > 0) {
                    if (rootPathString.slice(i, i + 1) === shiftStr.slice(i, i + 1)) {
                        i++
                        combineStr = rootPathString.slice(0, i)
                    } else {
                        break
                    }
                }
